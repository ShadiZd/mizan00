...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 10: Investment Engine
# ─────────────────────────────────────────────────────────────────────────────

class TestInvestmentEngine:
    """Tests for recommend_investments() in investment_engine.py."""

    # ── shared profiles ───────────────────────────────────────────────────────

    _SA_HALAL_MEDIUM = {
        "user_profile": {"risk_level": "medium", "region": "SA", "shariah_preference": True},
        "savings_summary": {"total_saved": 850.0, "monthly_savings_rate": 150.0},
    }

    # Test 1 — basic: returns at most 3 results
    def test_returns_at_most_three_recommendations(self):
        recs = recommend_investments(**self._SA_HALAL_MEDIUM)
        assert len(recs) <= 3

    # Test 2 — results are ranked 1, 2, 3
    def test_ranks_are_sequential(self):
        recs = recommend_investments(**self._SA_HALAL_MEDIUM)
        assert [r["rank"] for r in recs] == list(range(1, len(recs) + 1))

    # Test 3 — shariah filter: non-halal platforms excluded when preference=True
    def test_shariah_filter_excludes_non_compliant(self):
        recs = recommend_investments(
            user_profile={"risk_level": "medium", "region": "SA", "shariah_preference": True},
            savings_summary={"total_saved": 1000.0, "monthly_savings_rate": 200.0},
        )
        from platforms import investment_platforms
        non_halal = {p["name"] for p in investment_platforms if not p["shariah_compliant"]}
        returned_names = {r["platform"] for r in recs}
        assert returned_names.isdisjoint(non_halal), (
            f"Non-halal platforms appeared: {returned_names & non_halal}"
        )

    # Test 4 — region filter: SA-only platforms (no "global") excluded for non-SA users
    def test_sa_only_platform_excluded_for_non_sa_region(self):
        # Aghaz Invest only has ["SA"] — it should not appear for a UK user
        recs = recommend_investments(
            user_profile={"risk_level": "low", "region": "UK", "shariah_preference": True},
            savings_summary={"total_saved": 5000.0, "monthly_savings_rate": 500.0},
        )
        returned_names = [r["platform"] for r in recs]
        assert "Aghaz Invest" not in returned_names

    # Test 5 — min investment filter: platforms whose min > total_saved are excluded
    def test_min_investment_filter(self):
        # Nester requires 1000 SAR; with only 500 saved it should be excluded
        recs = recommend_investments(
            user_profile={"risk_level": "medium", "region": "SA", "shariah_preference": True},
            savings_summary={"total_saved": 500.0, "monthly_savings_rate": 100.0},
        )
        returned_names = [r["platform"] for r in recs]
        assert "Nester" not in returned_names

    # Test 6 — suggested_amount is 20% of total_saved
    def test_suggested_amount_is_20_percent_of_savings(self):
        total_saved = 850.0
        recs = recommend_investments(
            user_profile={"risk_level": "medium", "region": "SA", "shariah_preference": True},
            savings_summary={"total_saved": total_saved, "monthly_savings_rate": 150.0},
        )
        assert len(recs) > 0
        expected = round(total_saved * 0.20, 2)
        for r in recs:
            assert r["suggested_amount_sar"] == pytest.approx(expected, abs=0.01)

    # Test 7 — urgency "now" when total_saved > 500
    def test_urgency_now_when_savings_above_500(self):
        recs = recommend_investments(
            user_profile={"risk_level": "low", "region": "SA", "shariah_preference": True},
            savings_summary={"total_saved": 600.0, "monthly_savings_rate": 100.0},
        )
        assert len(recs) > 0
        assert all(r["urgency"] == "now" for r in recs)

    # Test 8 — urgency "soon" when 200 < total_saved <= 500
    def test_urgency_soon_when_savings_between_200_and_500(self):
        recs = recommend_investments(
            user_profile={"risk_level": "low", "region": "SA", "shariah_preference": True},
            savings_summary={"total_saved": 300.0, "monthly_savings_rate": 50.0},
        )
        assert len(recs) > 0
        assert all(r["urgency"] == "soon" for r in recs)

    # Test 9 — score increases when shariah preference matches
    def test_shariah_match_increases_score(self):
        halal_recs = recommend_investments(
            user_profile={"risk_level": "medium", "region": "SA", "shariah_preference": True},
            savings_summary={"total_saved": 850.0, "monthly_savings_rate": 0.0},
        )
        non_halal_recs = recommend_investments(
            user_profile={"risk_level": "medium", "region": "SA", "shariah_preference": False},
            savings_summary={"total_saved": 850.0, "monthly_savings_rate": 0.0},
        )
        # Wahed Invest (shariah=True) should score higher for halal users
        wahed_halal = next((r for r in halal_recs if r["platform"] == "Wahed Invest"), None)
        wahed_non_halal = next((r for r in non_halal_recs if r["platform"] == "Wahed Invest"), None)
        if wahed_halal and wahed_non_halal:
            assert wahed_halal["score"] > wahed_non_halal["score"]

    # Test 10 — pipeline investment_nudge is populated when contract is safe and savings > 200
    def test_pipeline_investment_nudge_when_safe_and_saved(self):
        result = _process_transaction(
            transaction_id="tx-invest-test",
            description="Starbucks coffee",
            amount=22.0,
            occurred_at=date(2026, 4, 15),
            use_ai=False,
        ...
        assert result.investment_nudge is not None
        assert result.investment_nudge["rank"] == 1
        assert "platform" in result.investment_nudge
        assert result.investment_nudge["suggested_amount_sar"] == pytest.approx(170.0, abs=0.01)
